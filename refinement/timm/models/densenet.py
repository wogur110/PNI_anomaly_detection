...
class DenseNet(nn.Module):
    ...
    def forward_densenet(self, x, pool=True):
        x = self.forward_densenet_get_early_features(x)
        x = self.forward_densenet_get_late_features(x, pool=pool)
        return x

    def forward_densenet_get_early_features(self, x):
    ...
    def forward_densenet_get_late_features(self, x, pool=True):
        denseblock1 = self.features.denseblock1(x)
        ...
        norm5 = self.features.norm5(denseblock4)
        out = F.relu(norm5, inplace=True)
        if pool:
            out = select_adaptive_pool2d(out, self.global_pool)
